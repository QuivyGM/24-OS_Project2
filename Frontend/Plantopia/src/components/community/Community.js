  ...
  const likesorting = (ind) => {
    //useEffect(() => {
      ...
      function cutString(str) {
        if(str.length > 35) return (str.substr(0,34) + "..."); else return (str);
      }
  ...
  return (
    <div className="community-page container">
      ...
      <h1 className="search-animation">
        ...
        <div className="forum-header">
          <div className="sort-dropdown">
            <select
              ...
              id="select"
              onChange={(e) => likesorting(document.getElementById('select').selectedIndex) }
            >
        ...
        {/* Post List */}
        <div id="PostListdiv" className="post-list">
          {dataList.map((post) => (
            <div
        ...
        >
            <div className="card-body">
                <div className="post-header">
                    ...
                    <span className="post-time">{getTimeDifference(post.time)}</span>
                </div>
                <h3 className="post-title">{post.title}</h3>
                <p className="post-content"> {cutString(post.contents)}</p>
